...
def summarise_action_result(reply: str, max_chars: int = MAX_PAGE_CHARS) -> str:
    ...
    # The reply is generated by our own browser code today, so `elements`
    # should always be a list -- but this is the one function standing between
    # a wire payload and the model's context, and "should" is not a guarantee
    # it can offer. A dict here would have raised TypeError straight through
    # _page_rpc's except and lost the turn. parse_listing is careful about
    # exactly this; so is this now.
    ...
    elements = [
        ...
        for item in (raw_elements[:MAX_ELEMENTS] if isinstance(raw_elements, list) else [])
    ...
    if not data.get("ok"):
        ...
        # A ref that moved or vanished is the one refusal the agent can act on
        # by itself, and the browser sends the current listing with it for
        # exactly that reason. Dropping it here would leave the agent knowing
        # only that it was wrong, with nothing to be right from -- which is
        # how a retry loop on a stale ref starts.
        if reason in ("ref_moved", "ref_not_found") and elements:
            return (
                f"REFUSED by the browser ({reason}): {detail}\n"
                f"{format_for_model(listing, max_chars=max_chars)}"
            )
