...
async def entrypoint(ctx: JobContext):
    ...
    # ── Avatar (provider-selected: flashhead default, bithuman legacy) ──
    # avatar_provider selects the engine. "flashhead" (default) renders via
    # the in-cluster flashhead-engine over WebSocket; "bithuman" is the legacy
    # self-hosted runtime. If flashhead is selected but no reference image is
    # available, fall back to bithuman when BitHuman creds are present so
    ...
    use_flashhead = (
        ...
        and bool(settings.flashhead_engine_url)
        and bool(flashhead_ref)
    )
    use_bithuman = (
        settings.avatar_enabled
        and not use_flashhead
        and bool(settings.bithuman_api_key)
    )
    ...
    if use_flashhead:
        ...
        try:
            ...
            logger.info(
                ...
                settings.flashhead_engine_url, avatar_name,
            )
        except ImportError:
            logger.warning(
                "livekit-plugins-flashhead not installed — avatar disabled, audio fallback ON"
            )
        except Exception as e:
            logger.warning("FlashHead avatar start failed: %s — audio fallback ON", e)
